			...
			switch (_ai_delegation_enabled) do {
				case 2: { //--- Headless Client delegation.
					...
					if (count _HC > 0) then {	
						...
						_groups = [];
						[_groups, group _soldier] call WFBE_CO_FNC_ArrayPush;
						_positions = [];
						[_positions, _position] call WFBE_CO_FNC_ArrayPush;
						_teams = [];
						[_teams, _team] call WFBE_CO_FNC_ArrayPush;
						[nil, side _soldier, _groups, _positions, _teams] Call WFBE_CO_FNC_DelegateAITownHeadless;
						//_soldier setOwner (owner (_HC select floor(random count _HC)));
					};
				};
			};
